...
# variables
FILES=(in out)
DIRECTORY=src/accepted/java
TEMPLATE=src/templates/template.java
...
if [ -f ${TEMPLATE} ]; then
  if [ "$#" -gt 0 ]; then
    echo $1.java
    timestamp | cat - ${TEMPLATE} > tmp && mv tmp $1.java
    echo "// Problem #    : "$1 | cat - $1.java > tmp && mv tmp $1.java
  fi
fi

# 2nd optional argument:
# use 2nd arg as file name (e.g. 123 for 123.java) to move to accepted
# DIRECTORY=src/accepted
if [ "$#" -eq 2 ]; then
  if [ -f $2.java ]; then
    echo moving $2.java to accepted

    # be more careful moving this
    if [ -d "$DIRECTORY" ]; then
      echo "directory exists"
      
      cp $2.java "$DIRECTORY"/$2.java
      if [ -f "$DIRECTORY"/$2.java ]; then
        rm $2.java
      else
        echo warning, $2.java did not copy to "$DIRECTORY" correctly.. skipping removal step
      fi
    fi
    ...
    if [ -f "Main.class" ]; then
      echo removing unneeded exe "Main.class"
      rm "Main.class"
    fi
  fi
fi
