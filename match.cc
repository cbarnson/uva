...
class MaxMatching {
public:
  static tuple<int, vi> max_matching2(const vector<vi> &g) {
    int n = 0, N = g.size();
    vi A(n, -1), D(N), used(N);
    
    for (auto &gg : g) for (int u : gg) n = max(n, u + 1);
  ...
  // bfs2
  static void bfs2(const vector<vi> &g, vi &used, vi &A, vi &D) {
    int n = g.size();
    D.assign(n, -1);
    vi Q(n);
    int q = 0;
  ...
  // dfs2
  static bool dfs2(const vector<vi> &g,
		   vi &vis, vi &used,
		   vi &match, vi &D, int u) {
    vis[u] = 1;
    for (int v : g[u]) {
