// Problem #    : main
// Created on   : 2018-06-15 14:48:18
...
bool BigInteger::isZero() const { return sign == 0; }
...
BigInteger fib(int n) {
  ...
  if (dp[n] != BigInteger(0)) return dp[n];

  int k = (n & 1) ? (n + 1) / 2 : n / 2;

  dp[n] = (n & 1) ? (fib(k) * fib(k) + fib(k - 1) * fib(k - 1))
                  : (2 * fib(k - 1) + fib(k)) * fib(k);

  return dp[n];
}

void printFibinary(BigInteger n) {
  BigInteger zero(0);
  int j = 2;
  if (n == 0 || n == 1) {
    cout << n;
...
int main() {
  ...
  while (cin >> a >> b) {
    if (tc > 1) cout << endl;
    tc++;
    BigInteger A = toFib(a), B = toFib(b), C;
    C = A + B;
    string ans = toFb(C);
    cout << ans << endl;
  }
