...
uint64_t solve(uint64_t& totalWays) {
  ...
  while (!pq.empty()) {
    ...
    if (ss.i == xt && ss.j == yt) {
      ...
      if (it != seen.end()) {
        // cout << "adding " << it->second.second << " ways for " << ss.d
        //      << " direction" << endl;
        ...
        if (shortestPath == -1) {
          ...
          uint64_t x = it->second.second;
          totalWays = (totalWays + x) % modfactor;
          hasPath = true;
        } else {
          // update condition
        ...
        // cout << it->second.ways << " ways for direction " << i << endl;
      }
      continue;
    }
