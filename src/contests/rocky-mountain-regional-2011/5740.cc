...
int main() {
   ...
   while (cin >> a >> b) {
	 ...
	 Date s, e;
	 cin >> s >> e;
	 trips[i].first = s.daysFromStart();
	 trips[i].second = e.daysFromStart();
      }

      int start_res = a.daysFromStart();
      int start_perm = b.daysFromStart();

      double diff = b.daysFromStart() - a.daysFromStart();

      // count this time, up to 730 days,  as half rounded down
      diff = min(diff, 730.0);      
      int half = floor(diff / 2);

      // int start = a.daysFromStart();
            
      vector<int> record;
      record.push_back(half);

      // fill vector.  Positive values indicate sections of residence and
      // negative values indicate trips outside of the country
      int last = start_perm;
      for (int i = 0; i < n; i++) {
