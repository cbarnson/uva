    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'div':
            if attrs == [('class', 'input')]:
                self.num_tests += 1
                print('%s/%s%d' % (self.folder, SAMPLE_INPUT, self.num_tests))
                self.testcase = open(
                    '%s/%s%d' % (self.folder, SAMPLE_INPUT, self.num_tests), 'w')
            elif attrs == [('class', 'output')]:
                print('%s/%s%d' % (self.folder, SAMPLE_OUTPUT, self.num_tests))
                self.testcase = open(
...
def parse_single_problem(contest, problem_letter):
    print('in parse_single_problem, contest: %s, problem letter %s' % (contest, problem_letter))
    # url = 'http://codeforces.com/contest/%s/problem/%s' % (contest, problem)
    url = 'http://codeforces.com/problemset/problem/%s/%s' % (contest, problem_letter)
    print('url: %s' % (url))
    html = urlopen(url).read()
    parser = CodeforcesSingleProblemParser
...
def main():
    # if(len(argv) < 2):
    #     print('USAGE: ./uleth_data.py 512')
    #     return
    # contest = argv[1]

    # --------------------------------------
    # some debug/demo python code
    #
    if (len(argv) < 3):
        print ('USAGE: ./dl.py abc filename')
        return 
    folder = argv[1]
    filename = argv[2]

    # This creates the directory, or multiple directories if folder = 'a/b/c' etc.
    call(['mkdir', '-p', folder])

    # copies our template to filename as per 2nd argument
    call(['cp', 'default_cf.cc', filename + '.cc'])
    print('done...')
    quit()
    # ---------------------------------------
    ...
    problem = "%s%s" % (contest, problem_letter)

    # Find contest and problems.
    print ('Parsing contest %s, please wait...' % contest)
    content = parse_contest(contest)
    print ('Done parsing contest.')

    # print ('Problem %s%s entered....' % (contest, problem_letter))
    # x = parse_single_problem(contest, problem_letter)

    for index, prob in enumerate(content.problems):
        print content.problem_names
        # print('downloading problem %s: %s...' % (prob, content.problem_names[index]))
        quit()
        
        print('Creating cpp source file...')
        call(['cp', TEMPLATE, '%s%s.cc' % (contest, problem_letter)])
        print('%d sample test(s) found.' % x)
        print ('========================================')    
        print ('Now quitting...')
        quit()
