...
#define setMatched(i) \
  (isU(i) ? (flagUmatched[(i)] = 1) : (flagVmatched[(i)-MAXU] = 1))
#define setUsed(i) (isU(i) ? (flagUused[(i)] = 1) : (flagVused[(i)-MAXU] = 1))
#define setVisited(i) \
  (isU(i) ? (flagUvisited[(i)] = 1) : (flagVvisited[(i)-MAXU] = 1))
  ...
  do { /* find alternating paths by repeated bfs. */
    ...
    while (head != tail) {
      ...
      if (isV(i) && !isMatched(i)) {
        ...
        for (j = 0; j < v; j++) {
          if (bipgraph[i][j] && !isVisited(V(j)) && !isUsed(V(j)) &&
              matching[i] != j) {
...
int main() {
  ...
  while (fscanf(in, "%d %d", &u, &v) == 2) {
    ...
    for (i = 0; i < u; i++) {
      if (matching[i] != -1) fprintf(out, "match %d to %d\n", i, matching[i]);
    }
  }
  return 0;
}
